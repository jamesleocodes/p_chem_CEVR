...
best_parameters_with_lc = {'learning_rate': 0.09452466774062965,
 'max_depth': 8.327013432602142,
 'min_samples_leaf': 0.10407128524456705,
 'min_samples_split': 0.12108256709953003,
 'n_estimators': 3,
 'subsample': 0.8531412362699697}
n_estimators_list = [50, 100, 200, 300, 400, 500]
best_model = GradientBoostingRegressor(learning_rate=best_parameters_with_lc['learning_rate'],
                subsample=best_parameters_with_lc['subsample'],
                max_depth=best_parameters_with_lc['max_depth'],
                min_samples_split=best_parameters_with_lc['min_samples_split'],
                min_samples_leaf=best_parameters_with_lc['min_samples_leaf'],
                n_estimators=n_estimators_list[best_parameters_with_lc['n_estimators']],random_state=4
                )
best_model.fit(data_tra_x,data_tra_y)
tes_pred = best_model.predict(data_tes_x)

plt.scatter(tes_pred,data_tes_y,c='r',alpha=1)
plt.xlabel('Predicted LogP by GB',fontsize = 14)
plt.ylabel('Experimental LogP',fontsize = 14)
plt.savefig("gb_vs.png", format='png', dpi=600)
plt.show()
...
train_sizes, train_scores, validation_scores = learning_curve(estimator = GradientBoostingRegressor(learning_rate=best_parameters_with_lc['learning_rate'],
                subsample=best_parameters_with_lc['subsample'],
                max_depth=best_parameters_with_lc['max_depth'],
                min_samples_split=best_parameters_with_lc['min_samples_split'],
                min_samples_leaf=best_parameters_with_lc['min_samples_leaf'],
...
ax.set_title('Learning curve for a GB model', fontsize = 14, y = 1.03)
ax.legend()
plt.savefig("gb_lc.png", format='png', dpi=600)
plt.show()
