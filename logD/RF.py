"""
Author: Zaw
Predicting the LogD of molecules by using RF
"""
...
def run_best_model(arg_1,arg_2):
    ...
    all_set_df.to_excel(dirname+"/p_chem_logd/results/rf_single_no_rt.xlsx")
...
def run_best_model(arg_1,arg_2):
    ...
    final.to_excel(dirname+"/p_chem_logd/results/rf_50_no_rt.xlsx")
run_best_model(des_df,lc_df)
...
path = os.getcwd()
dirname = os.path.dirname(path)
file_path = dirname+"/p_chem_logd/hyperparameters/rf_lc.pkl"
...
def run_best_model(arg_1,arg_2):
    ...
    all_set_df = pd.DataFrame(all_set,columns=['set','mse','rmse','mae','r2'])
    all_set_df.to_excel(dirname+"/p_chem_logd/results/rf_single_rt.xlsx")
...
def run_best_model(arg_1,arg_2):
    ...
    final = pd.DataFrame(data_res,columns = ['', 'Training',' Validation', 'Testing'])
    final.to_excel(dirname+"/p_chem_logd/results/rf_50_rt.xlsx")
